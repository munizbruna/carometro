...
DROP PROCEDURE IF EXISTS PopulateAlunos;
DELIMITER //
CREATE PROCEDURE PopulateAlunos()
BEGIN
    ...
    DECLARE celular VARCHAR(45);
    DECLARE cep VARCHAR(8);
    DECLARE logradouro VARCHAR(100);
    DECLARE bairro VARCHAR(45);
    ...
    WHILE i <= 192 DO
        ...
        SET email = CONCAT('aluno', i, '@exemplo.com');
        SET cpf = LPAD(i, 11, '0');
        SET senha = 'senha';
        SET celular = CONCAT('9900000', LPAD(i, 3, '0'));
        SET cep = '12345678';
        SET logradouro = CONCAT('Rua ', CHAR(64 + i MOD 26));
        SET bairro = CONCAT('Bairro ', CHAR(64 + i MOD 26));
        SET cidade = CONCAT('Cidade ', CHAR(64 + i MOD 26));
        SET estado = CONCAT('E', CHAR(64 + i MOD 26));
        SET imagem = CONCAT('foto', i, '.jpg');

        INSERT INTO `carometro`.`usuarios` (nome, email, cpf, senha, celular, cep, logradouro, bairro, cidade, estado, Tipos_Usuarios_idTipos_Usuarios, imagem)
        VALUES (nome, email, cpf, senha, celular, cep, logradouro, bairro, cidade, estado, 1, imagem);
...
DELIMITER //
DROP PROCEDURE IF EXISTS AssociateUsuariosTurmas;
CREATE PROCEDURE AssociateUsuariosTurmas()
BEGIN
    DECLARE user_id INT DEFAULT 1;
    DECLARE turma_id INT DEFAULT 1;
    DECLARE count INT DEFAULT 0;

    WHILE user_id <= 192 DO
        IF count = 32 THEN
            SET count = 0;
            SET turma_id = turma_id + 1;
        END IF;

        INSERT INTO `carometro`.`usuarios_turmas` (Usuarios_idUsuarios, Turmas_idTurmas)
        SELECT idUsuarios, turma_id
        FROM `carometro`.`usuarios`
