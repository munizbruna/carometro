const multer = require('multer');
const path = require('path');
const crypto = require('crypto');
const Usuarios = require('../models/usuarios')


// Função para gerar um nome de arquivo único
const generateUniqueFilename = (originalName) => {
    const ext = path.extname(originalName);
    const name = path.basename(originalName, ext);
    const uniqueSuffix = crypto.randomBytes(8).toString('hex');
    return `${name}-${uniqueSuffix}${ext}`;
};

// Configuração do multer para armazenar os uploads no diretório especificado
const storage = multer.diskStorage({
    destination: function (req, file, cb) {
        cb(null, path.join(__dirname, '..', 'uploads')); // Caminho relativo usando __dirname
    },
    filename: function (req, file, cb) {
        cb(null, generateUniqueFilename(file.originalname));
    }
});

// Filtro para aceitar apenas arquivos de imagem
const imageFileFilter = (req, file, cb) => {
    const allowedTypes = /jpeg|jpg|png|gif/;
    const extname = allowedTypes.test(path.extname(file.originalname).toLowerCase());
    const mimetype = allowedTypes.test(file.mimetype);

    if (extname && mimetype) {
        return cb(null, true);
    } else {
        cb(new Error('Apenas arquivos de imagem são permitidos.'));
    }
};

const upload = multer({
    storage: storage,
    limits: { fileSize: 10 * 1024 * 1024 }, // Limite de tamanho do arquivo (10 MB)
    fileFilter: imageFileFilter
}).single('image');

// Função para lidar com o upload de arquivo
const handleFileUpload = (req, res, next) => {
    upload(req, res, function (err) {
        if (err) {
            return res.status(400).send(`Erro no upload do arquivo: ${err.message}`);
        }
        if (!req.file) {
            return res.status(400).send('Nenhum arquivo enviado ou o nome do campo do formulário está incorreto.');
        }

        // Salve os dados no banco de dados usando Sequelize ou qualquer outra biblioteca de ORM
        let uploadedFilePath; // Variável para armazenar o caminho do arquivo

        const handleFileUpload = (req, res, next) => {
            upload(req, res, function (err) {
                if (err) {
                    return res.status(400).send(`Erro no upload do arquivo: ${err.message}`);
                }
                if (!req.file) {
                    return res.status(400).send('Nenhum arquivo enviado ou o nome do campo do formulário está incorreto.');
                }
                
                // Salve o caminho do arquivo na variável
                uploadedFilePath = req.file.filename;
        
                // Envie uma resposta de sucesso com o caminho do arquivo
                res.json({ message: 'Arquivo enviado com sucesso!', filePath: req.file.filename });
            });
        };
        
        // Exporte a variável para uso em outros arquivos
        module.exports = { handleFileUpload, uploadedFilePath };
        
    });
};

module.exports = { handleFileUpload };
